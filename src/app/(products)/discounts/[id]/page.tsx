...
const ProductPage : React.FC<CardProductProps> = ({params}:any) => {
    ...
    const navigation= [
        {"names":"Descuentos","url":"/discounts"},
        {"names":data?.nombre ?? "Not found","url":"/discounts/"+(data?.id ?? "")}
    ]
        ...
        ">
            ...
            ">
                ...
                "> 
                    <div>
                        ...
                        <ul className="grid grid-cols-4 m-1 gap-1">
                            <li><Image className="rounded-sm" src={image} alt={data?.descripcion ?? ""}></Image></li>
                            <li><Image className="rounded-sm" src={image} alt={data?.descripcion ?? ""}></Image></li>
                            <li><Image className="rounded-sm" src={image} alt={data?.descripcion ?? ""}></Image></li>
                            <li><Image className="rounded-sm" src={image} alt={data?.descripcion ?? ""}></Image></li>
                        </ul>
                ...
                "> 
                    ...
                    <div>
                        <h3 className="text-red-mafer uppercase font-bold text-lg
                            xl:text-xl
                        "> {data?.nombre ?? "Not found"} </h3>
                        <ul className="flex flex-row text-sm
                            xl:text-base
                        "> 
                            <li> ⭐⭐⭐⭐⭐ </li>
                        </ul>
                        <h4 className="my-2 text-red-mafer font-bold text-2xl
                            xl:text-3xl
                        "> {data?.precio ?? "Not found"}  </h4>
                    ...
                    ">
                        <li className="basis-2/6">
                            <span className="icon icon-shop text-2xl
                                xl:text-4xl
                            "></span>
                            <p> Reclama en nuestro punto. </p>
                        </li>
                        <li className="basis-2/6">
                            <span className="icon icon-ranking text-2xl
                                xl:text-4xl
                            "></span>
                            <p> Calidad garantizada. </p>
                        </li>
                        <li className="basis-2/6">
                            <span className="icon icon-truck text-2xl
                                xl:text-4xl
                            "></span>
